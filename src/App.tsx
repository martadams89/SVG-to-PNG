...
export default function App() {
  ...
  const generatePreview = useCallback(() => {
    ...
    img.onload = () => {
      ...
      // Draw the specific portion (crop) at the specified scale
      ctx.drawImage(
        img, 
    ...
    img.src = url;
  }, [svgContent, scale, crop]);

  useEffect(() => {
    if (svgContent) {
      generatePreview();
    }
  }, [svgContent, scale, crop, generatePreview]);
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <header className="mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-[#9e9e9e] mt-1">Convert, scale, and crop with precision.</p>
          </div>
          {svgContent && (
            <button 
              onClick={reset}
          ...
          <div className="lg:col-span-8 space-y-6">
            {!svgContent ? (
              <div 
                {...getRootProps()} 
                className={`
                  h-[400px] border-2 border-dashed rounded-3xl flex flex-col items-center justify-center cursor-pointer transition-all
                  ${isDragActive ? 'border-emerald-500 bg-emerald-50/30' : 'border-[#d1d1d1] hover:border-[#9e9e9e] bg-white/50'}
                `}
              >
                <input {...getInputProps()} />
              ...
              <div className="bg-white rounded-3xl shadow-sm border border-black/5 overflow-hidden flex flex-col h-[600px]">
                <div className="p-4 border-b border-black/5 flex items-center justify-between bg-[#fafafa]">
                  ...
                  <div className="flex items-center gap-4 text-xs text-[#9e9e9e]">
                    <span>Original: {Math.round(originalSize.width)} x {Math.round(originalSize.height)}</span>
                    <span>Output: {Math.round(crop.width * scale)} x {Math.round(crop.height * scale)}</span>
                  </div>
                </div>
              ...
              {svgContent ? (
                ...
                >
                  ...
                  <div className="bg-white p-6 rounded-3xl shadow-sm border border-black/5 space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm font-mono bg-[#f5f5f5] px-2 py-1 rounded-md">{scale.toFixed(2)}x</span>
                    </div>
                      ...
                      {[0.5, 1, 2, 4].map((s) => (
                        <button 
                          ...
                          className={`
                            ...
                            ${scale === s ? 'bg-emerald-600 text-white shadow-md shadow-emerald-200' : 'bg-[#f5f5f5] text-[#9e9e9e] hover:bg-[#f0f0f0]'}
                          `}
                        >
                          {s}x
                        </button>
                  ...
                  <div className="bg-white p-6 rounded-3xl shadow-sm border border-black/5 space-y-4">
                    <div className="flex items-center justify-between">
                      ...
                      <button 
                        ...
                        className={`text-xs px-3 py-1 rounded-full border transition-all ${isCropping ? 'bg-emerald-600 text-white border-emerald-600' : 'text-[#9e9e9e] border-[#d1d1d1]'}`}
                      >
                        {isCropping ? 'Editing' : 'Edit'}
                      </button>
                    </div>

                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1">
                        <label className="text-[10px] uppercase tracking-wider text-[#9e9e9e] font-semibold">X Offset</label>
                        <input 
                          type="number" 
                      ...
                      <div className="space-y-1">
                        <label className="text-[10px] uppercase tracking-wider text-[#9e9e9e] font-semibold">Y Offset</label>
                        <input 
                          type="number" 
                      ...
                      <div className="space-y-1">
                        <label className="text-[10px] uppercase tracking-wider text-[#9e9e9e] font-semibold">Width</label>
                        <input 
                          type="number" 
                          value={Math.round(crop.width)}
                          onChange={(e) => setCrop({ ...crop, width: parseInt(e.target.value) || 0 })}
                          className="w-full px-3 py-2 bg-[#f5f5f5] rounded-xl text-sm focus:outline-none focus:ring-1 focus:ring-emerald-600"
                        />
                      </div>
                      <div className="space-y-1">
                        <label className="text-[10px] uppercase tracking-wider text-[#9e9e9e] font-semibold">Height</label>
                        <input 
                          type="number" 
                          value={Math.round(crop.height)}
                          onChange={(e) => setCrop({ ...crop, height: parseInt(e.target.value) || 0 })}
                          className="w-full px-3 py-2 bg-[#f5f5f5] rounded-xl text-sm focus:outline-none focus:ring-1 focus:ring-emerald-600"
                        />
                      </div>
                    </div>
                    ...
                    <button 
                      onClick={() => setCrop({ x: 0, y: 0, width: originalSize.width, height: originalSize.height })}
                ...
                <div className="bg-white p-8 rounded-3xl shadow-sm border border-black/5 flex flex-col items-center text-center space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-[#9e9e9e] mt-1">Upload an SVG to start customizing your export.</p>
                  </div>
                </div>
              )}
            </AnimatePresence>
